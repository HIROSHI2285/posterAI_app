...
【フォントとテキスト品質（厳守）】
1. 日本語テキストには必ず日本語対応フォント（Noto Sans JP、游ゴシック、ヒラギノ角ゴ、源ノ角ゴシック、Meiryo、MS ゴシックなど）を使用してください
2. ポスターの雰囲気・目的に最適なフォントスタイルを選択してください（例：ポップなら丸ゴシック、フォーマルなら明朝体、モダンならゴシック体）
3. 文字が重なったり欠けたりしないよう、十分な余白とレイアウトを確保してください
4. テキストサイズは読みやすさを優先し、視認性を確保してください
5. 文字化けや判読不能なフォントは絶対に使用しないでください
6. タイトル、サブタイトル、本文で適切な文字サイズの階層を設けてください
