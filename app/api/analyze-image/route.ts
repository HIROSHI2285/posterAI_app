...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            const prompt = `あなたはプロのデザイン分析官です。この画像を解析し、Imagen 3 が効果的に再現できるように、要点を絞ったデザイン仕様書を作成してください。
無駄な装飾語は省き、具体的な形状、質感、色の名前（HEX）を優先して記述してください。

【出力制限】
- 必ず JSON 形式で出力してください。
- 以下のJSON形式で回答してください（JSONのみ、他のテキストは含めないでください）。
- コストとパフォーマンスの観点から、各項目の記述量は合計で **1500〜2000文字程度** に収まるように要約・凝縮してください。
- 長すぎる出力はエラーになるため、最も重要な「レイアウト構造」「テキスト配置」「配色」を優先してください。

{
  ...
  "detailedDescription": "以下の構造で、AI画像生成（Imagen）で再現するための指示として記述してください。合計1500〜2000文字程度に要約してください：
