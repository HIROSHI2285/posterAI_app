    ...
    try {
        ...
        if (textEdits && textEdits.length > 0) {
            ...
            if (deletions.length > 0) {
                promptParts.push('')
                promptParts.push('='.repeat(50))
                promptParts.push('【重要: テキストの削除（最優先）】')
                promptParts.push('='.repeat(50))
                promptParts.push('')
                promptParts.push('以下のテキストを画像から完全に消去し、跡形もなく消してください。')
                promptParts.push('**削除はデザイン上の意図的な決定です。背景と周囲のデザインに完全に馴染むように、高精度なインペイント処理を行ってください。**')
                promptParts.push('')
                deletions.forEach((edit, i) => {
                    promptParts.push(`${i + 1}. 「${edit.original}」を画像から消去（ピクセルを除去）`)
                })
                promptParts.push('')
                promptParts.push('**厳守事項**:')
                promptParts.push('1. 指定された文字を完全に消し去ってください。ぼかしや不完全な上塗りは不可です。')
                promptParts.push('2. 削除箇所は周囲の背景テクスチャ・デザインを推測して自然に修復してください。')
                promptParts.push('3. 削除による空白が不自然にならないよう、周囲と完全に調和させてください。')
                promptParts.push('')
                promptParts.push('='.repeat(50))
            }
        }
