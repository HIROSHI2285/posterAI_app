...
- 🎛️ **AIモデル選択機能 (v1.4.0)**:
  - **本番モデル**: 高品質・サイズ遵守 (`gemini-3-pro-image-preview`)
  - **開発モデル**: 高速・低コスト (`gemini-2.5-flash-image`)
- 💾 **高画質ダウンロード (v1.4.0)**: Canvas APIによるクライアントサイド・アップスケールで高解像度保存
- 🎛️ **画像参照強度コントロール**: 3段階で調整可能（強い/普通/弱い）
- 🎭 **カスタマイズ可能**: カラーパレット、レイアウト、テイストなど細かく調整可能
- ✏️ **画像編集機能**: 生成後にプロンプトで部分編集（背景変更、色調整等）
- 🎨 **マスク編集機能**: マウスでなぞった領域だけをAIで編集
  - 最大5領域を同時に指定可能
  - Canvas描画でブラシサイズ調整
  - 「1: タイトル変更、2: 日付変更」形式で指示
- 📝 **テキスト編集機能（NEW）**: 生成後のテキストを直接編集
  - Gemini Visionでテキスト座標を自動抽出
  - ダブルクリックで文字内容を変更
  - 色・サイズ・フォントをプロパティパネルで調整
- 📎 **画像挿入機能**: ロゴや画像を最大5枚まで挿入・差し替え可能
- 🔐 **高度なセキュリティ対策 (v1.4.0)**:
  - CSP (Content Security Policy) と違反レポート
  - 監査ログとレート制限
  - セキュアなエラーハンドリング
- 👥 **ユーザー管理**: 管理者による許可制アクセス管理
- 📚 **使い方ガイド (v1.4.0)**: AI生成モックアップを用いた、視覚的で分かりやすい完全ガイド
...
3. **環境変数の設定**
   ...
   # 画像編集モデル（デフォルト）
   # ユーザーはUI上で「本番モデル(gemini-3-pro-image-preview)」と「開発モデル(gemini-2.5-flash-image)」を選択可能
   # ここでの設定はフォールバックとして使用されます
